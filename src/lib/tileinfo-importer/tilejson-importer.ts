...
import { SourceSpecification, LayerSpecification } from '@maplibre/maplibre-gl-style-spec/types'
...
export class TileJSONImporter extends BaseImporter {
    async getJSON (url: string): Promise<TileInfoJSONResponse> {
        ...
        const layers : LayerSpecification[] = this.getVectorLayers(tilesetName, tilejson.vector_layers)
