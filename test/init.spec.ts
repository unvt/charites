...
describe('Test for the `init.ts`.', () => {
  ...
  it('Should initialize default decomposited style.yml from tilejson provided', async() => {
    ...
    assert.deepEqual(true, !! fs.statSync(styleYaml))
    // the file should be the same with init_tilejson.yml
    assert.deepEqual(fs.readFileSync(styleYaml, 'utf8'), fs.readFileSync(path.join(__dirname, 'data/init/tilejson/init_decomposite.yml'), 'utf-8'))
    assert.deepEqual(YAML.load(fs.readFileSync(path.join(tmpdir, 'layers/bicycle_parking.yml'), 'utf8')), YAML.load(fs.readFileSync(path.join(__dirname, 'data/init/tilejson/layers/bicycle_parking.yml'), 'utf-8')))
    assert.deepEqual(YAML.load(fs.readFileSync(path.join(tmpdir, 'layers/showers.yml'), 'utf8')), YAML.load(fs.readFileSync(path.join(__dirname, 'data/init/tilejson/layers/showers.yml'), 'utf-8')))
    assert.deepEqual(YAML.load(fs.readFileSync(path.join(tmpdir, 'layers/telephone.yml'), 'utf8')), YAML.load(fs.readFileSync(path.join(__dirname, 'data/init/tilejson/layers/telephone.yml'), 'utf-8')))
  });
  ...
  it('Should initialize default decomposited style.yml from metadatajson provided', async() => {
    ...
    const options: initOptions = {
      metadatajsonUrls : 'https://optgeo.github.io/kokoromi-rw/zxy/metadata.json',
      compositeLayers: true
    }
