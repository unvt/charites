...
describe('Test for the `build.ts`.', () => {
  ...
  it('Should minify `data/style.yml` to JSON.', async () => {
    ...
    await build(styleYaml, styleJson, {provider: defaultValues.provider, compactOutput: true})
  ...
  it('Should build command replace sprite url with the option `--sprite-url`.', async () => {
    ...
    assert.deepEqual(expectedUrl, JSON.parse(fs.readFileSync(styleJson, 'utf-8')).sprite)
  });
